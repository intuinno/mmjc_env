...
class GoalType(IntEnum):
    """Goal types for taxi navigation (3 goals)."""
    FORWARD = 0
    ROTATE_CW = 1      # Clockwise (negative yaw rate)
    ROTATE_CCW = 2     # Counter-clockwise (positive yaw rate)


class GoalType4(IntEnum):
    """Goal types for taxi navigation (4 goals)."""
    FORWARD = 0
    ROTATE_CW = 1      # Clockwise (negative yaw rate)
    ROTATE_CCW = 2     # Counter-clockwise (positive yaw rate)
    BACKWARD = 3       # Move backward
...
class TaxiNavigationTask4Goal(composer.Task):
    """dm_control task for taxi-style goal following with 4 goals."""
    ...
    def should_terminate_episode(self, physics):
        aliveness = self._walker.aliveness(physics)
        return aliveness < -0.5


class TaxiNavigation4GoalEnv(gymnasium.Env):
    """Gymnasium environment for taxi-style navigation with 4 goals.

    Goals are: FORWARD, ROTATE_CW, ROTATE_CCW, BACKWARD.

    Observations:
        - goal: One-hot encoding of current goal (4,)
        - proprioception: Sensor data from the walker
    ...
    def __init__(
        ...
        self.observation_space = spaces.Dict({
            "goal": spaces.Box(0, 1, shape=(4,), dtype=np.float32),
            "proprioception": spaces.Box(
                -np.inf, np.inf,
                shape=(self._proprioception_dim,),
                dtype=np.float32
    ...
    def _render_human(self):
        if self._window is None:
            ...
            pygame.display.init()
            self._window = pygame.display.set_mode((480, 480))
            pygame.display.set_caption("TaxiNavigation4GoalEnv")
        if self._clock is None:
            self._clock = pygame.time.Clock()
        ...
        goal_colors = {
            GoalType4.FORWARD: (0, 255, 0),      # Green
            GoalType4.ROTATE_CW: (255, 165, 0),  # Orange
            GoalType4.ROTATE_CCW: (0, 191, 255), # Blue
            GoalType4.BACKWARD: (255, 0, 255),   # Magenta
        }
        goal_names = {
            GoalType4.FORWARD: "FORWARD",
            GoalType4.ROTATE_CW: "ROTATE CW",
            GoalType4.ROTATE_CCW: "ROTATE CCW",
            GoalType4.BACKWARD: "BACKWARD",
        }
        goal = self._task.current_goal
        goal_text = font.render(
            f"Goal: {goal_names[goal]}", True, goal_colors[goal]
...
class TaxiNavigationTask4GoalDistance(composer.Task):
    """dm_control task for taxi-style goal following with 4 goals.

    Uses a windowed distance/angle reward: accumulates position and heading
    changes over a fixed window (default 40 steps), then delivers a single
    reward at the end of the window. Steps within the window receive 0 reward.
    """
    ...
    def __init__(
        ...
        self._window_start_heading = None
    ...
    def _get_position(self, physics):
        ...
        return physics.bind(self._walker.root_body).xpos[:2].copy()

    def _get_heading(self, physics):
        """Get yaw angle (heading) from the walker root body's rotation matrix."""
        xmat = physics.bind(self._walker.root_body).xmat.reshape(3, 3)
        yaw = np.arctan2(xmat[1, 0], xmat[0, 0])
        return yaw

    def _reset_window(self, physics):
        """Reset window tracking state."""
        self._window_step = 0
        self._window_start_pos = self._get_position(physics)
        self._window_start_heading = self._get_heading(physics)
    ...
    def get_reward(self, physics):
        ...
        current_heading = self._get_heading(physics)

        dx = current_pos[0] - self._window_start_pos[0]
        dy = current_pos[1] - self._window_start_pos[1]

        # Forward displacement in the direction the walker was facing at window start
        start_h = self._window_start_heading
        forward_disp = dx * np.cos(start_h) + dy * np.sin(start_h)

        # Heading change (signed, wrapped to [-pi, pi])
...
class TaxiNavigation4GoalDistanceEnv(gymnasium.Env):
    """Gymnasium environment for taxi navigation with 4 goals and windowed distance/angle reward.

    Goals are: FORWARD, ROTATE_CW, ROTATE_CCW, BACKWARD.
    Reward is computed over a window of steps (default 40) based on actual
    displacement (for FORWARD/BACKWARD) or heading change (for rotations).
    ...
    def _render_human(self):
        if self._window is None:
            ...
            pygame.display.init()
            self._window = pygame.display.set_mode((480, 480))
            pygame.display.set_caption("TaxiNavigation4GoalDistanceEnv")
        if self._clock is None:
            self._clock = pygame.time.Clock()
        ...
        goal_colors = {
            GoalType4.FORWARD: (0, 255, 0),
            GoalType4.ROTATE_CW: (255, 165, 0),
            GoalType4.ROTATE_CCW: (0, 191, 255),
            GoalType4.BACKWARD: (255, 0, 255),
        }
        goal_names = {
            GoalType4.FORWARD: "FORWARD",
            GoalType4.ROTATE_CW: "ROTATE CW",
        ...
        window_text = font.render(
            f"Window: {self._task._window_step}/{self._task.reward_window}", True, (255, 255, 255)
        )
        self._window.blit(window_text, (10, 190))
...
class TaxiNavigation4GoalVectorEnv(gymnasium.Env):
    ...
    GOAL_VECTORS = {
        GoalType4.FORWARD: np.array([+1.0, 0.0], dtype=np.float32),
        GoalType4.ROTATE_CW: np.array([0.0, -1.0], dtype=np.float32),
        GoalType4.ROTATE_CCW: np.array([0.0, +1.0], dtype=np.float32),
        GoalType4.BACKWARD: np.array([-1.0, 0.0], dtype=np.float32),
    }
    ...
    def __init__(
        ...
        self.observation_space = spaces.Dict({
            "goal": spaces.Box(-1, 1, shape=(2,), dtype=np.float32),
            "proprioception": spaces.Box(
                -np.inf, np.inf,
                shape=(self._proprioception_dim,),
                dtype=np.float32
    ...
    def _get_goal_vector(self) -> np.ndarray:
        return self.GOAL_VECTORS[self._task.current_goal].copy()

    def _transform_observation(self, dm_obs):
        return {
            "goal": self._get_goal_vector(),
            "proprioception": self._get_proprioception(dm_obs),
        }
    ...
    def _render_human(self):
        if self._window is None:
            ...
            pygame.display.init()
            self._window = pygame.display.set_mode((480, 480))
            pygame.display.set_caption("TaxiNavigation4GoalVectorEnv")
        if self._clock is None:
            self._clock = pygame.time.Clock()
        ...
        goal = self._task.current_goal
        goal_vec = self._get_goal_vector()
        goal_text = font.render(
            f"Goal: {goal_names[goal]} [{goal_vec[0]:+.0f},{goal_vec[1]:+.0f}]",
            True, goal_colors[goal]
        )
        self._window.blit(goal_text, (10, 10))
