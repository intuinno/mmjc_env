...
class MazeEnv(gym.Env):
    ...
    def __init__(
        ...
        self.action_space = spaces.Dict(
            {
                "leg_FL": spaces.Box(-1, 1, shape=(4,)),
                "leg_FR": spaces.Box(-1, 1, shape=(4,)),
                "leg_RL": spaces.Box(-1, 1, shape=(4,)),
                "leg_RR": spaces.Box(-1, 1, shape=(4,)),
                "neck": spaces.Box(-1, 1, shape=(4,)),
                "focus": spaces.Box(0, 1, shape=(self.num_focus,), dtype=np.uint8),
            }
        )
        self.observation_space = spaces.Dict(
            {
                "peripheral": spaces.Box(
                    0,
                    255,
                    shape=(self.peri_size, self.peri_size, self.num_channel),
                    dtype=np.uint8,
                ),
                "central": spaces.Box(
                    0,
                    255,
                    shape=(self.central_size, self.central_size, self.num_channel),
                    dtype=np.uint8,
                ),
                "focus": spaces.Box(0, 1, shape=(self.num_focus,), dtype=np.uint8),
                "head_sensor": spaces.Box(-np.inf, np.inf, shape=(4,)),
                "head_motor": spaces.Box(-np.inf, np.inf, shape=(3,)),
                "body_sensor": spaces.Box(-np.inf, np.inf, shape=(4,)),
                "leg_FR_motor": spaces.Box(-np.inf, np.inf, shape=(4,)),
                "leg_FR_sensor": spaces.Box(-np.inf, np.inf, shape=(6,)),
                "leg_FL_motor": spaces.Box(-np.inf, np.inf, shape=(4,)),
                "leg_FL_sensor": spaces.Box(-np.inf, np.inf, shape=(6,)),
                "leg_RR_motor": spaces.Box(-np.inf, np.inf, shape=(4,)),
                "leg_RR_sensor": spaces.Box(-np.inf, np.inf, shape=(6,)),
                "leg_RL_motor": spaces.Box(-np.inf, np.inf, shape=(4,)),
                "leg_RL_sensor": spaces.Box(-np.inf, np.inf, shape=(6,)),
            }
        )
    ...
    def step(self, action) -> Tuple[Any, float, bool, dict]:
        ...
        if done and not terminal:
            info["TimeLimit.truncated"] = (
                True  # acme.GymWrapper understands this and converts back to dm_env.truncation()
            )
        return ts.observation, ts.reward, done, info

    def _build_obs(self, dm_obs):
        print(dm_obs)
...
class VecMazeEnvAdapter:
    def __init__(self, num_envs, configs):
        ...
        dm_env = tasks.memory_maze_15x15(
