...
from mmjc_env.memory_maze.wrappers import ObservationWrapper
...
class DrawMinimapWrapper(ObservationWrapper):
    ...
    def observation(self, obs):
        ...
        map[:, :] += (maze == 1)[..., None] * np.array(
            [[[255, 255, 255]]], np.uint8
        )  # corridors in white
        map[:, :] += (maze == 2)[..., None] * np.array(
            [[[0, 255, 0]]], np.uint8
        )  # path in green
        ...
        tx = (x - N / 2) / N * SIZE
        ty = -(y - N / 2) / N * SIZE
        mapimg = mapimg.transform(mapimg.size, 0, (1, 0, tx, 0, 1, ty), resample=0)
...
def breadth_first_search(
    maze: np.ndarray, start: Tuple[int, int], finish: Tuple[int, int]
